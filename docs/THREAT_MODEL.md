...
| A stolen token becomes a general process-control capability | The compiled policy permits only the exact Roblox singleton event and has no arbitrary inspect endpoints | A same-user caller can still disrupt Roblox or consume another client's dry-run plan |
...
| Stale or recycled handle | The engine rechecks kernel-object identity immediately before `DuplicateHandle(..., DUPLICATE_CLOSE_SOURCE)` | Windows exposes no atomic compare-and-close primitive |
| Broad, malformed, or resource-heavy requests | An 8 KiB strict JSON reader rejects unknown, duplicate, incorrectly cased, or malformed fields; Kestrel limits headers, timeouts, connections, and upgrades | Repeated same-user requests can still cause bounded local work |
| Close without review or request replay | Execution requires an identical successful dry run; the in-memory plan expires after five seconds and is consumed once | A same-user attacker with the token can race to consume a known plan, causing disruption or denial of service |
